...
class Blockchain:
    def __init__(self):
        ...
        self.validators = set()  # Registered validators
    ...
    def register_validator(self, name):
        self.validators.add(name)
...
registration_type = st.sidebar.radio("Register as:", ("Validator", "User"))
name = st.sidebar.text_input("Name")
if st.sidebar.button("Register"):
    if registration_type == "Validator":
        st.session_state.blockchain.register_validator(name)
        st.sidebar.success(f"Validator {name} registered!")
    else:
        st.session_state.blockchain.register_user(name)
        st.sidebar.success(f"User {name} registered!")

# Sidebar for staking coins (only for validators)
st.sidebar.header("Stake Coins")
validator_name = st.sidebar.text_input("Validator Name (for staking)")
stake_amount = st.sidebar.number_input("Stake Amount", min_value=1, value=10)
if st.sidebar.button("Stake"):
    if validator_name in st.session_state.blockchain.validators:
        ...
        st.sidebar.error("Only registered validators can stake coins.")

# Main area for creating posts and viewing blockchain
st.header("Create a New Post")
user_name = st.text_input("Your Name (must be a registered user)")
post_content = st.text_area("Post Content")
if st.button("Submit Post"):
    if user_name in st.session_state.blockchain.users:
        st.session_state.blockchain.create_transaction(f"{user_name}: {post_content}")
        st.success("Post added to pending transactions!")
    else:
        st.error("Only registered users can create posts.")
